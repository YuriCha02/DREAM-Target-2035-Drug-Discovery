...
class GNNStack(nn.Module):
    def __init__(self, input_dim, hidden_dim, args, emb=False, output_dim = 1, use_maccs=True):
        ...
        if use_maccs:
            raw_aux_dim = 2 + 8  # MW, ALOGP, and 8-dim MACCS embedder
        else:
            raw_aux_dim = 2      # MW, ALOGP only

        self.aux_proj = nn.Sequential(
            nn.Linear(raw_aux_dim, hidden_dim),
            nn.ReLU(),
            nn.Dropout(self.dropout)
        )

        total_input_dim = gnn_out_dim + hidden_dim  # GNN + projected aux
    ...
    def forward(self, data):
        ...
        if graph_features:
            aux = torch.cat(graph_features, dim=1)
