# DREAM Target 2035 Drug Discovery Challenge (Bioryzm)

This repository contains the code for Graph Attention Network (GAT) used in the challenge

9th place in 1st round, 13th place in 2nd round

Team member: $\text{Luka Lisica}^{1,2}, \text{Dejan Lisica}^{3}, \text{Pratyusha Ran}^{4}, \text{Mmazule Igirigba}^{5}, \text{Yuri Cha}^{6}$
1. Institute for Oncology and Radiology of Serbia
2. Faculty of Medicine, University of Novi Sad
3. Faculty of Technical Sciences, University of Novi Sad
4. St. Xavier's College, Mumbai
5. University of Port Harcourt
6. National Canter center Graduate School of Cancer Science and Poilcy

DREAM-SGC Target 2035 Challenge: https://www.synapse.org/#!Synapse:syn65660836

Our team wiki: https://www.synapse.org/Synapse:syn66720603/wiki/632627
